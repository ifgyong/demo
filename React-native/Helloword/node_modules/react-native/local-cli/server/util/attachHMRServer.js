...
:
    ...
    Promise.all(response.dependencies.map(async(dep
:
    TModule
)
    =
...
:
    ...
    if (arrayEquals(deps, oldDependencies)) {
        ...
        packagerServer.getDependencies({
            dev: true,
            entryFile: filename,
            rootEntryFile: client.bundleEntry,
            hot: true,
...
:
    ...
    function sendFileChangeToClient(
        client
...
:
    ...
    if (type !== 'delete') {
        const response = await
        prepareResponse(client, filename);
