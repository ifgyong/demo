...
function saveAsIndexedFile(
    ...
    const
        ...
        bundleOutput =
            ...
            options.bundleOutput, encoding = options.bundleEncoding, sourcemapOutput = options.sourcemapOutput,
        sourcemapSourcesRoot = options.sourcemapSourcesRoot;

    log('start');
    var _bundle$getUnbundle =
        bundle.getUnbundle();
    const startupModules = _bundle$getUnbundle.startupModules, lazyModules = _bundle$getUnbundle.lazyModules,
        groups = _bundle$getUnbundle.groups;
    log('finish');
    ...
    const writeUnbundle = writeBuffers(
        ...
        buildTableAndContents(startupCode, lazyModules, moduleGroups, encoding)).then(() = > log('Done writing unbundle output')
)
    ;
...
function buildModuleBuffers(modules, moduleGroups, encoding) {
    return modules.filter(m = > !moduleGroups.modulesInGroups.has(m.id)
).
    map((_ref2) = > {let id = _ref2.id, code = _ref2.code;
...
function buildTableAndContents(
    ...
    return [
        fileHeader,
        table,
        startupCodeBuffer].concat(moduleBuffers.map((_ref3) = > {let buffer = _ref3.buffer;
    return buffer;
}))
    ;
}
