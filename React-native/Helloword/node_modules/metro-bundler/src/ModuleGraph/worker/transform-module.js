...
var _require2 =
    require('path');
const basename = _require2.basename;
...
function transformModule(
    ...
    const
        ...
        filename = options.filename, transformer = options.transformer, polyfill = options.polyfill;
    var _options$variants = options.variants;
    const variants = _options$variants === undefined ? defaultVariants : _options$variants;
    const transformed = {};

    for (const variantName of Object.keys(variants)) {
        var _transformer$transfor =
        ...
        const ast = _transformer$transfor.ast;
...
function getAssetSize(type, content) {
    ...
    const width = _getImageSize.width, height = _getImageSize.height;
    return {width, height};
}

function makeResult(ast, filename, sourceCode) {
    let isPolyfill = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
    let dependencies, dependencyMapName, file;
    if (isPolyfill) {
        dependencies = [];
        file = JsFileWrapping.wrapPolyfill(ast);
    } else {
        var _collectDependencies =
