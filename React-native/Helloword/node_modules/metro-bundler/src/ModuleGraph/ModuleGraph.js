...
exports.createBuildSetup = function (
    ...
    postProcessModules) {
    let
        translateDefaultsPath = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : x =
>
    x;
    return (() = > {
        var _ref = _asyncToGenerator(
            function* (entryPoints, options) {
                var _options$getPolyfills =


                    options.getPolyfills;
                const getPolyfills = _options$getPolyfills === undefined ? function (_ref5) {
                    let platform = _ref5.platform;
                    return [];
                } : _options$getPolyfills;
                var _options$optimize = options.optimize;
                const optimize = _options$optimize === undefined ? false : _options$optimize;
                var _options$platform = options.platform;
                const platform = _options$platform === undefined ? defaults.platforms[0] : _options$platform;
                const graphOptions = {optimize};

                const graphWithOptions = function (entry) {
                    return graphFn(entry, platform, graphOptions);
                };
                const graphOnlyModules = (() = > {
                    var _ref2 = _asyncToGenerator(function* (m) {
                        return (yield graphWithOptions(m)).modules;
                    });
                return function graphOnlyModules(_x4) {
                ...
                var _ref3 =
                        ...
                        yield Promise.all([
                            _asyncToGenerator(function* () {
                                const result = yield graphWithOptions(entryPoints);
                                const
                ...
                const graph = _ref4[0], moduleSystem = _ref4[1], polyfills = _ref4[2];
                const
