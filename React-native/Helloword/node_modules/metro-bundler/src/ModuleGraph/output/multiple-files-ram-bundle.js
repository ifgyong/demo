...
var _require =
    ...
    require('./util');
const concat = _require.concat, getModuleCode = _require.getModuleCode, partition = _require.partition,
...
function asMultipleFilesRamBundle(_ref) {
    let filename = _ref.filename, idsForPath = _ref.idsForPath, modules = _ref.modules,
        requireCalls = _ref.requireCalls, preloadedModules = _ref.preloadedModules;
    const idForPath = x =
>
    ...
    const code = startupModules.map(m = > getModuleCode(m, idForPath)
).
    join('\n');
...
function createBuilder(
    ...
    return x =
>
    asMultipleFilesRamBundle(_extends({}, x, {preloadedModules, ramGroupHeads}));
}
