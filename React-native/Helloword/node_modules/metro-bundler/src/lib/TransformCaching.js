...
class FileBasedCache {
    ...
    _getCacheFilePaths(props) {
        const hasher = crypto.createHash('sha1').update(props.filePath).update(props.transformOptionsKey);
...
function readMetadataFileSync(
    ...
    var _metadata = _slicedToArray(
        metadata, 5);
    const cachedResultHash = _metadata[0], cachedSourceHash = _metadata[1], dependencies = _metadata[2],
        dependencyOffsets = _metadata[3], sourceMap = _metadata[4];
    if (
...
function hashSourceCode(props) {
    return crypto.createHash('sha1').update(props.getTransformCacheKey(props.transformOptions)).update(props.sourceCode).digest('hex');
}
