...
class KeyResultStore {
    ...
    _processResults(keyResults) {
        var _this3 = this;
        return _asyncToGenerator(function* () {
            const resultsByKey = new Map(
                keyResults.map(function (pair) {
                    return [pair.key, pair.result];
                }));
...
function profileKey(_ref) {
    let dev = _ref.dev, minify = _ref.minify, platform = _ref.platform;
...
class URIBasedGlobalTransformCache {
    ...
    keyOf(props) {
        ...
        hash.update(
            crypto.createHash('sha1').update(sourceCode).digest('hex'));
     ...
     * megabytes each.
     */
    static _fetchResultFromURI(uri) {
        return _asyncToGenerator(function* () {
    ...
    static _fetchResultFromURIWithRetry(uri) {
        return URIBasedGlobalTransformCache._fetchResultFromURI(
            uri).catch(error = > {
            if(
        !URIBasedGlobalTransformCache.shouldRetryAfterThatError(error)
    )
        {
            throw error;
        }
...
class OptionsHasher {
    ...
    hashTransformWorkerOptions(
        ...
        options) {
        const
            dev = options.dev, minify = options.minify, platform = options.platform, transform = options.transform,
            unknowns = _objectWithoutProperties(options, ['dev', 'minify', 'platform', 'transform']);
    ...
    hashTransformOptions(
        ...
        const
            ...
            enableBabelRCLookup =
                ...
                options.enableBabelRCLookup, generateSourceMaps = options.generateSourceMaps, dev = options.dev,
            hot = options.hot, inlineRequires = options.inlineRequires, platform = options.platform,
            projectRoot = options.projectRoot,
            unknowns = _objectWithoutProperties(options, ['enableBabelRCLookup', 'generateSourceMaps', 'dev', 'hot', 'inlineRequires', 'platform', 'projectRoot']);
        const unknownKeys = Object.keys(unknowns);
        if (unknownKeys.length > 0) {
