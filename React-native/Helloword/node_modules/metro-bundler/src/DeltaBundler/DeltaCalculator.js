...
class DeltaCalculator extends EventEmitter {
    ...
    >
        ...
        this._dependencyGraph.getWatcher().on('change', this._handleMultipleFileChanges);
    }

    /**
     * Stops listening for file changes and clears all the caches.
     */end() {
        this._dependencyGraph.getWatcher().removeListener('change', this._handleMultipleFileChanges); // Clean up all the cache data structures to deallocate memory.
        this._modifiedFiles = new Set();
        this._deletedFiles = new Set();
        this._dependencyEdges = new Map();
    }
     ...
     */getDelta() {
        ...
        return _asyncToGenerator(function* () {// If there is already a build in progress, wait until it finish to start
            ...
            _this._currentBuildPromise = _this._getChangedDependencies(modifiedFiles, deletedFiles);
            let result;
            const numDependencies = _this._dependencyEdges.size;
            try {
                result = yield _this._currentBuildPromise;
            } catch (error) {// In case of error, we don't want to mark the modified files as
                ...
                if (_this._dependencyEdges.size !== numDependencies) {
                    _this._dependencyEdges = new Map();
                }
                throw error;
            } finally {
     ...
     */getTransformerOptions() {
        ...
        return _asyncToGenerator(function* () {
            if (!_this2._transformerOptions) {
                _this2._transformerOptions = (yield _this2._bundler.getTransformOptions(_this2._options.entryFile, {
                    dev: _this2._options.dev,
                    generateSourceMaps: false,
                    hot: _this2._options.hot,
                    minify: _this2._options.minify,
                    platform: _this2._options.platform,
                    prependPolyfills: false
                })).transformer;
            }
            return _this2._transformerOptions;
        })();
    }
     ...
     */_getChangedDependencies(modifiedFiles, deletedFiles) {
        var _this3 = this;
        return _asyncToGenerator(function* () {
            const transformerOptions = yield _this3.getTransformerOptions();
            if (!_this3._dependencyEdges.size) {
                const path = _this3._dependencyGraph.getAbsolutePath(_this3._options.entryFile);
                const modified = new Map([[path, _this3._dependencyGraph.getModuleForPath(path)]]);
                var _ref3 = yield initialTraverseDependencies(path, _this3._dependencyGraph, transformerOptions, _this3._dependencyEdges, _this3._options.onProgress || undefined);
                const added = _ref3.added;
                for (const path of added) {
                    modified.set(path, _this3._dependencyGraph.getModuleForPath(path));
                }
                return {modified, deleted: new Set(), reset: true};
            }
            ...
            deletedFiles.forEach(function (filePath) {
                ...
                if (edge) {
                    edge.inverseDependencies.forEach(function (path) {
                        return modifiedFiles.add(path);
                    });
                }
            });

            // We only want to process files that are in the bundle.
            const modifiedDependencies = Array.from(modifiedFiles).filter(function (filePath) {
                return (
            ...
            var _ref4 =
                ...
                yield traverseDependencies(
                    ...
                    _this3._options.onProgress || undefined);
            const added = _ref4.added, deleted = _ref4.deleted;
