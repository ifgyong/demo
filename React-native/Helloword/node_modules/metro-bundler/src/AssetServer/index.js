...
class AssetServer {
    ...
    _getAssetRecord(
        ...
        return this._findRoot(this._roots, path.dirname(assetPath), assetPath).then(dir = > Promise.all([dir, readDir(dir)])
    ).
        then(res = > {
            const dir = res[0];
        const files = res[1];
    ...
    >
        ...
        const rootsString = roots.map(s = > `'${s}'`
    ).
        join(', ');
        throw new Error(
...
function hashFiles(files, hash, callback) {
    ...
    fs.createReadStream(files.shift()).on('data', data = > hash.update(data)
).
