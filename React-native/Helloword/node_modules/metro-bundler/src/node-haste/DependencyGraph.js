...
var _require =
    ...
    require('../Logger');
const createActionEndEntry = _require.createActionEndEntry, createActionStartEntry = _require.createActionStartEntry,
    log = _require.log;
var _require2 =
    require('./DependencyGraph/ModuleResolution');
const ModuleResolver = _require2.ModuleResolver;
var _require3 =
    require('events');
const EventEmitter = _require3.EventEmitter;
...
class DependencyGraph extends EventEmitter {
    ...
    >
        ...
        this._assetResolutionCache = new AssetResolutionCache({
            ...
            getDirFiles: dirPath = > this._filesByDirNameIndex.getAllFiles(dirPath), platforms
    :
        config.opts.platforms
    })
    ...
    static load(opts) {
        return _asyncToGenerator(function* () {
            ...
            const hasteFS = _ref.hasteFS, moduleMap = _ref.moduleMap;
            log(createActionEndEntry(log(createActionStartEntry('Initializing Metro Bundler'))));
            opts.reporter.update({type: 'dep_graph_loaded'});
            return new DependencyGraph({haste, initialHasteFS: hasteFS, initialModuleMap: moduleMap, opts});
        })();
    }
    ...
    _onHasteChange(_ref2) {
        ...
        eventsQueue.forEach((_ref3) = > {let type = _ref3.type, filePath = _ref3.filePath;
    ...
    _createModuleResolver() {
        this._moduleResolver = new ModuleResolver({dirExists: filePath = > {try {return fs.lstatSync(filePath).isDirectory();
    } catch
        (e)
        {
        }
        ...
        doesFileExist: this._doesFileExist, extraNodeModules
    :
        this._opts.extraNodeModules, helpers
    :
        this._helpers, moduleCache
    :
        this._moduleCache, moduleMap
    :
        this._moduleMap, preferNativePlatform
    :
        this._opts.preferNativePlatform, resolveAsset
    :
        (dirPath, assetName, platform) =
    >
        this._assetResolutionCache.resolve(dirPath, assetName, platform), sourceExts
    :
        this._opts.sourceExts
    })
        ;
    }
    ...
    resolveDependency(fromModule, toModuleName, platform) {
    ...
    getDependencies(_ref4) {
        let entryPath = _ref4.entryPath, options = _ref4.options, platform = _ref4.platform,
            onProgress = _ref4.onProgress;
        var _ref4$recursive = _ref4.recursive;
        let recursive = _ref4$recursive === undefined ? true : _ref4$recursive;
