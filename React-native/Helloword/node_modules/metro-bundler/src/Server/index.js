...
var _require =
    ...
    require('../Logger');
const createActionStartEntry = _require.createActionStartEntry, createActionEndEntry = _require.createActionEndEntry,
    log = _require.log;
...
class Server {
    ...
    >
        {
            let type = _ref.type, filePath = _ref.filePath;
            return (
        ...
        this._bundler.getResolver().then(resolver = > {
            resolver.
                getDependencyGraph().getWatcher().on('change', (_ref2) = > {let eventsQueue = _ref2.eventsQueue;
        return (
    ...
    buildBundle(options) {
        ...
        return _asyncToGenerator(function* () {
            ...
            const nonVirtual = modules.filter(function (m) {
                return !m.virtual;
            });
            bundleDeps.set(bundle, {
                files: new Map(
                    nonVirtual.map(function (_ref3) {
                        let sourcePath = _ref3.sourcePath, meta = _ref3.meta;
                        return [
                            sourcePath,
                ...
                idToIndex: new Map(modules.map(function (_ref4, i) {
                    let id = _ref4.id;
                    return [id, i];
                })),
                dependencyPairs: new Map(
                    nonVirtual.filter(function (_ref5) {
                        let meta = _ref5.meta;
                        return meta && meta.dependencyPairs;
                    }).map(function (m) {
                        return [m.sourcePath, m.meta.dependencyPairs];
                    })),
    ...
    getShallowDependencies(options) {
        ...
        const
            ...
            entryFile =
                ...
                options.entryFile, dev = options.dev, minify = options.minify, hot = options.hot,
            rootEntryFile = options.rootEntryFile, bundlingOptions = options.bundlingOptions;
    ...
    getDependencies(
        ...
        const
            ...
            rootEntryFile = options.rootEntryFile;
    ...
    _rangeRequestMiddleware(
        ...
        if (req.headers && req.headers.range) {
            ...
            const rangeStart = _req$headers$range$re2[0], rangeEnd = _req$headers$range$re2[1];
    ...
    >
        ...
        if (optionsJson in this._bundles) {
            return this._bundles[optionsJson].then(bundle = > {
                const deps = bundleDeps.get(bundle);
            // $FlowFixMe(>=0.37.0)
            const dependencyPairs = deps.dependencyPairs, files = deps.files, idToIndex = deps.idToIndex,
            ...
            if (outdated.size) {
                ...
                const bundlePromise = this._bundles[optionsJson] = Promise.all([
                    ...
                    Promise.all(Array.from(outdated, this.getModuleForPath, this))]).then((_ref6) = > {var _ref7 = _slicedToArray(_ref6, 2);
                let response = _ref7[0], changedModules = _ref7[1];
                            ...
                            return Promise.reject(
                                Error(
                                    `Dependencies of ${sourcePath} changed from [${/* $FlowFixMe: `get` can return empty */
                                        files.get(sourcePath).join(', ')}] to [${meta.dependencies.join(', ')}]`));
    ...
    processRequest(
        ...
        return _asyncToGenerator(function* () {
            ...
            building.then(
                ...
                function (error) {
                    return _this3._handleError(mres, _this3.optionsHash(options), error);
    ...
    _processDeltaRequest(req, res) {
        ...
        return _asyncToGenerator(function* () {
            const mres = MultipartResponse.wrap(req, res);
            var _prepareDeltaBundler =
                _this4._prepareDeltaBundler(req, mres);
            const options = _prepareDeltaBundler.options, buildID = _prepareDeltaBundler.buildID;
    ...
    _processBundleUsingDeltaBundler(
        ...
        return _asyncToGenerator(function* () {
            const mres = MultipartResponse.wrap(req, res);
            var _prepareDeltaBundler2 =
                _this5._prepareDeltaBundler(req, mres);
            const options = _prepareDeltaBundler2.options, buildID = _prepareDeltaBundler2.buildID;
    ...
    _processSourceMapUsingDeltaBundler(
        ...
        return _asyncToGenerator(function* () {
            const mres = MultipartResponse.wrap(req, res);
            var _prepareDeltaBundler3 =
                _this6._prepareDeltaBundler(req, mres);
            const options = _prepareDeltaBundler3.options, buildID = _prepareDeltaBundler3.buildID;
    ...
    _sourceMapForURL(reqUrl) {
        ...
        return _asyncToGenerator(function* () {
            ...
            return building.then(function (p) {
                return (
                    p.getSourceMap({
                        excludeSource: options.excludeSource,
                        minify: options.minify,
    ...
    _getOptionsFromUrl(reqUrl) {
        ...
        const entryFile =
            pathname.replace(/^\//, '').split('.').filter(part = > {
                if(
                    part === 'includeRequire' ||
            part === 'runModule' ||
            part === 'bundle' ||
